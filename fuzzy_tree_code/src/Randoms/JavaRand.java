...
public class JavaRand extends java.util.Random {
    ...
    /**
     * Return a random BitSet with "size" bits, each having probability p of
     * being true.
     */
    ...
    /**
     * Return a random double in the range 0 to 1, inclusive, uniformly sampled
     * from that range. The mean of this distribution is 0.5. The variance is
     * 1/12.
     */
     ...
     */
    public double random(boolean isAuto) {
        if (isAuto) {
            return Math.random();
        } else {
            return nextUniform();
     ...
     */
    public double random(double low, double high, boolean isAuto) {
        if (isAuto) {
            return (double) (Math.random() * (high - low) + low);//take from system
        } else {
            return (double) (random(false) * (high - low) + low);//take from seed
        }
    }//random
     ...
     */
    public int random(int low, int high, boolean isAuto) {
        if (isAuto) {
            return (int) (Math.random() * (high - low) + low);//take from system
        } else {
            return (int) (random(false) * (high - low) + low);//take from seed
        }
    }//random 
    /**
     * 
     ...
     */
    public int random(int range, boolean isAuto) {
        if (isAuto) {
            return (int) (Math.floor(Math.random() * range));
        } else {
            return (int) (Math.floor(random(false) * range));
        }
    }//random
     ...
     * @param isAuto take random from system: true/false
     * @return a vector of random numbers between min and max
     */
    public int[] randomIntVector(int min, int max, boolean isAuto) {
        int a[] = new int[max];
        for (int i = 0; i < max; i++) {
            if (i == 0) {
                a[i] = random(min, max - 1,isAuto);
            } else {
                while (true) {
                    boolean flag = false;
                    int r = random(min, max - 1,isAuto);
                    for (int j = 0; j < i; j++) {
                        if (r == a[j]) {
                            flag = true;
                            break;
